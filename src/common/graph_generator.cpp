...
void graph_generator::original_to_boost() {
	...
	boost::graph_traits<BoostJohnsonGraph>::vertex_descriptor u3, v3;
	...
	for (size_t i = 0; i < m_size; i++) {
		for (size_t j = 0; j < i; j++) {
			if (m_original_graph[i][j] != NO_EDGE) {
				...
				u3 = boost::vertex(i, m_boost_johnson_graph);
				v3 = boost::vertex(j, m_boost_johnson_graph);
				boost::add_edge(u3, v3, m_original_graph[i][j], m_boost_johnson_graph);
			}
		}
	}
}
...
graph_generator::BoostJohnsonGraph graph_generator::boost_johnson_graph() {
